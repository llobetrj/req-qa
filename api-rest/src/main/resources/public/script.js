...
function getDataFromAPI(reqs) {
    ...
    const url = "http://localhost:8089/analyze";

    let responses = [];
    const numLines = reqs.length;
    let divWrapper = $('#ambiguity-wrapper');
    if (divWrapper) {
        divWrapper.remove();
    }


    $("#body").append(`<div id="ambiguity-wrapper">
        <hr>
    ...
    $.ajax({
            ...
            processResponse(reqs, jQuery.parseJSON(response));
...
function processResponse(reqs, response) {
    ...
    $.each(response, function (lineIndex, ambiguities) {
        ...
        if (indexCharacters(reqIndex, lineIndex, reqs[lineIndex]))
            ...
            addHeader(useCaseNumber++);
        ambStats.push(highlightCharacters(lineIndex, ambiguities.requirementAnalysisDetailList));
    });
